        ...
        <div className="font-display bg-background-dark text-slate-100 min-h-screen selection:bg-primary/30 relative overflow-hidden">
            ...
            <div className="relative z-10 p-8 lg:p-12">
                ...
                <header className="mb-10">
                    <h2 className="text-4xl font-black text-white tracking-tight uppercase mb-2">Application Reviews</h2>
                ...
                <div className="glass-card rounded-2xl overflow-hidden mb-8">
                    {/* Table Header */}
                    <div className="grid grid-cols-[1fr_auto] p-6 border-b border-white/10 text-xs font-bold text-slate-400 uppercase tracking-wider">
                        <div>Applicant</div>
                            ...
                            {pendingUsers.map((applicant) => (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-white font-bold text-base">{applicant.name}</h3>
                                            ...
                                            <div className="flex items-center gap-3 mt-1">
                                                <span className="text-xs bg-white/5 border border-white/10 px-2 py-0.5 rounded-full text-slate-300">{applicant.dbRole}</span>
                                                <span className="text-xs text-slate-500">{applicant.department}</span>
                                                <span className="text-xs text-slate-500">{applicant.jobRole}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                                        <button
                                            ...
                                            className="px-6 py-2 rounded-full border border-accent-teal/50 text-accent-teal hover:bg-accent-teal/10 font-semibold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            {actionLoading === applicant._id ? 'Processing...' : 'Approve'}
                                        </button>
                                        <button
                                            onClick={() => handleReject(applicant._id)}
                                            disabled={actionLoading === applicant._id}
                                            className="px-6 py-2 rounded-full border border-rose-500/50 text-rose-500 hover:bg-rose-500/10 font-semibold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Reject
                                        </button>
                                    </div>
